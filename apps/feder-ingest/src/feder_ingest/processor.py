...
class Processor:
                ...
                case RMQCommand() as cmd:
                    match cmd.message:
                        case rmq.DataMessage() as msg:
                            ...
                            self.process_trajectory_batch(batch)
                        ...
                        case rmq.RPCMessage() as msg:
                            match msg.endpoint:
                                case 'liveness:ingester':
                                    ...
                                    if self.rmq is None:
                                        logger.warning('cannot answer liveness RPC without RMQ')
                                        continue
    ...
    def process_trajectory_batch(self, batch: TrajectoryBatch) -> None:
        if len(batch.trajectories) == 0:
            # This marks the end of a day, so we can promote the current day's
            # in-memory database to a file.
            self.db.end_of_day(date.fromordinal(batch.trajectory_count))
            return
        ...
        # If something goes wrong saving an individual trajectory, DO NOT make
        # the whole ingester fail!  Just log the error and mark it in the error
        # monitoring metric.  This stops us losing any data from OK
        # trajectories in the batch following a bad one.
        any_failed = False
        ...
        # Make sure the trajectory count is monotonically increasing! If
        # batches get delivered out of order and we don't do this, it can
        # confuse the flow control logic in the receiver.
